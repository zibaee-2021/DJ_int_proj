...
- (Incidentally, one can directly visualise atomic structures of two or more models of a protein for which NMR data has 
  ...
  - <details><summary><strong>RMSD and TM-score matrices and clustering:</strong></summary> 
    ...
    |------------------------------|--------------------------------------------------------|--------------------------------------------------------|
    | Basic idea                   | Average Euclidean distance after optimal superposition | Length-normalised, distance-weighted structural similarity |
...
- <details><summary><strong>src/</strong></summary><br>
  ...
  - <details><summary><strong>Essential dynamics (ED):</strong></summary><br>
    ...
    Equivalent, per-element format (as coded in `build_data_matrix()`): 
    $R_{ij} = \frac{C_{ij}}{\sigma_i \, \sigma_j}$
    
    The eigendecomposition is done (using a NumPy built-in function) to produce the eigenvalue(s), each of which tells 
    you how much an ensemble fluctuates along that mode; and the corresponding eigenvector(s) (of shape 3N, where N is 
    the number of atoms)), each of which tells you the actual pattern of motion in terms of which atoms move where, when 
    the protein moves along that mode. Scores generated by projecting each model to the eigenvectors indicates how much 
    each model conforms to each mode. These scores help identify protein structural conformers. 
    ...
    For example, if a lot of the variance is explain by the first principal component such that:
  ...
  - <details><summary><strong>Normal Mode Analysis (NMA):</strong></summary><br>
    ...
    NMA relies on the eigendecomposition of a Hessian matrix to obtain collective vibrational modes of a system.
    ...
    **Elastic network models (ENMs)** are a simplified form of NMA in which interatomic interactions are replaced by 
    ...
    | ANM                                            | $3N \times 3N$                    | Per-residue 3D displacement vectors (coarse-grained)              | **Yes**: residue-level directions                           | Predicted collective 3D directions of residue motion (hinges, domain rotations)                            |
    | PCA / ED (MD or NMR)                           | $3N \times 3N$ covariance         | Dominant directions of *observed* Cartesian variance              | **Yes**: but data-driven, not predictive                    | Dominant *observed* displacement directions sampled experimentally or in MD                                |
    ...
    | DDM                                            | $N \times N$ (pairwise distances) | Complete internal geometric change between structures             | **Implicitly yes**: internal geometry only, no global frame | Complete information about which internal distances change and by how much; no Cartesian directions        |
    | GNM                                            | $N \times N$ Laplacian            | Scalar participation / correlation patterns of residues           | **No**: isotropic, sign structure only                      | Predicted relative flexibility and correlated/anti-correlated regions                                      |
    ...
    | Concept/Object     | **Normal Mode (for NMA)**                                                               | **Essential Mode (for PCA/ED)**                          |
    |--------------------|-----------------------------------------------------------------------------------------|----------------------------------------------------------|
    ...
    - <details><summary><strong>Protein sequence alignments using MMseqs2:</strong></summary><br>
      ...
      - <details>
        <summary>`mmseqs2.py`</summary>
          ...
          `pident`: The percent identity over the alignment is given by: 
           <p align="center">
           $\dfrac{\text{Number of identical residues in alignment}}{\text{alignment length}} \times 100$
           </p>
...
<details><summary>Using an LLM for code generation:</summary><br>
  ...
  This is the first codebase in which I have made wholesale use of an LLM for building groups of fully-written functions. 
  ...
  I have also used an LLM here for:
  ...
  - LLM was used for writing the inital core functionalities in:
  ...
  - LLM was used for writing the clustering & visualisations in [RMSD.py](src/RMSD.py)
