...
- The exploration of the NMR dataset naturally led to the question of how one determines the existence of different 
  ...
  - <details><summary><strong>RMSD and TM-score matrices and clustering:</strong></summary> 
    ...
    software package ('EnsembleFlex'), in a very recent publication (Schneider et al. 2025.)
...
- <details><summary><strong>data/</strong></summary><br>

  - Data files were not staged in git (but can all be generated by running code in `src` dir). 
  About 99% of the data is in `NMR` dir. `DynDom` includes 239 data scripts sent by Prof Hayward, amongst other files. 
  The `XRAY` dir has a handful of haemoglobin structures, downloaded ad hoc for testing one of the scripts 
  (`diff_distance_matrix.py`). Below is the full data directory structure and brief descriptions of files in each dir and subdir:
